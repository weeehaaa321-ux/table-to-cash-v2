...
async function reassignSweep(realId: string, currentShift: number) {
  // Waiter app off: no waiter-to-table assignment, so the sweep is a
  // no-op. Saves the throttled DB roundtrip on every poll. Toggling
  // back to enabled: the next poll reactivates the sweep and any
  // orphan sessions (existing or freshly opened) get adopted by the
  // next eligible waiter.
  const waiterAppOn = await readWaiterAppEnabled(realId);
  if (!waiterAppOn) return;
